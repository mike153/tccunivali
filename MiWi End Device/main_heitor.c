...
//unsigned short carga_dispositivo = 65000;
/********************** FUNCTION (&)PROTOTYPES *************************/
...
void trata_int_e_timer() 
{
	if(INTCON3bits.INT1IF) //Verifica se a INT1 aconteceu
    {
        ...
        if (carga_dispositivo > 0) 
        	WriteTimer3(carga_dispositivo);
        
    }
        
    if (PIR2bits.TMR3IF && T3CONbits.TMR3ON) 
    {
	    PIR2bits.TMR3IF = 0;
		CloseTimer3();
	...
	*pRxData++;     //take off the seq number
	...
	switch(*pRxData++)      //report type
	{
		case USER_REPORT_TYPE:
			...
			{
				case 0x86:
	//				ConsolePutROMString((ROM char*)"Desligou\r\n");
					desliga();
					TxPayLoad();
					WriteData(USER_REPORT_TYPE);
				...
				break;
				case 0x87:
	//				ConsolePutROMString((ROM char*)"Ligou\r\n");
					liga();
					TxPayLoad();
					WriteData(USER_REPORT_TYPE);
...
{   
	...
	TICK tickDifference;
	BYTE myFriend = 0xFF;
	unsigned short teste = carga_disp(20);
...
void BoardInit(void)
{
	...
	TRISB = 0x13; //RB1 como input - INT1
	LATB = 0x00;
	PORTB = 0x00;
	...
	INTCON2bits.INTEDG1 = 1; //Boarda de subida
